...
import {
  ...
  Download, X, Package, Trash2, List, MapPin
} from 'lucide-react'
...
import LocationTreeView from '../components/Assets/LocationTreeView'
...
export default function AssetRegisterPage() {
  ...
  const exportCSV = (onlySelected = false) => {
    const source = onlySelected
      ? visibleAssets.filter(a => selectedRows.includes(a.id))
      : visibleAssets
    if (onlySelected && source.length === 0) return
    const rows = [
    ...
    a.href = url
    a.download = onlySelected ? 'qspiders-assets-selected.csv' : 'qspiders-assets.csv'
    a.click()
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-56">
          ...
          <input
            type="text"
            placeholder="Search by name, ID, serial, location, custodian…"
            value={search}
        ...
        <div className="flex items-center rounded-lg border border-slate-200 bg-white p-0.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-medium transition ${viewMode === 'table' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}
          >
            <List size={15} />
            Table
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-medium transition ${viewMode === 'location' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}
          >
            <MapPin size={15} />
            By Location
          </button>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">
          ...
          {viewMode === 'table' && selectedRows.length > 0 && (
            <span className="ml-2 text-orange-600">· {selectedRows.length} selected</span>
          )}
        </p>
        {viewMode === 'table' && selectedRows.length > 0 && (
          <div className="flex items-center gap-2">
            <button onClick={() => exportCSV(true)} className="text-xs flex items-center gap-1 px-3 py-1.5 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600">
      ...
      {viewMode === 'location' ? (
        <LocationTreeView assets={visibleAssets} />
      ) : (
